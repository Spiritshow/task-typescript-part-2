...
//     if (!key || value === undefined || value === undefined) return undefined;
//   return items.find(item => item[key] === value);
// }

function findInArray<T>(items: T[], findobj: Partial<T>): T | undefined {
  return items.find(item => Object.entries(findobj).every(([key, value]) => item[key as keyof T] === value));
}
...
const foundUser = findInArray(users,{name: "Alice", age: 25});
