...
function mapAndFilter<T,U>(items: T[], transform: (item: T ) => U, filter: (item: U) => boolean): U[] {
  return items.map(item => transform(item)).filter(filter);// Ваш код здесь
}
...
// function mapAndFilter<T,U>(items: T[], transform: (item: T ) => U, filter: (item: U) => boolean, sortFoo: (a: U, b: U) => number): U[] {
//   const res = items.map(item => transform(item)).filter(filter);
//   return sortFoo? res.sort(sortFoo) : res;
// }
// Могут ли типы T и U быть полностью разными или должны иметь общие характеристики? Объясните ваш ответ.
// мгут быть полностью разными. Потому что связь осуществляется через переданную нами функцию изменения transform
//---------------------------------------------------------------------------------
